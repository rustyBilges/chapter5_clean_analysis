...
plot_map(AX[0], 'el_sync_map_mai_0'+strad+'.csv', mmax=0, mmin=-7, xoff=True, row_lab = 'MAI = 0.0', col_lab='el sync', ylab='immigration', annot='A', log=True)
plot_map(AX[1], 'el_inv_map_mai_0'+strad+'.csv', mmax=15, xoff=True,yoff=True, col_lab='el inv', annot='B', log=True)   
plot_map(AX[2], 'pop_inv_map_mai_0'+strad+'.csv', mmax=10, xoff=True, yoff=True, col_lab='pop inv', annot='C', log=True) 

#plot_map(AX[3], 'min_inv_map_mai_0.5'+strad+'.csv', mmax=5, mmin=-9, xoff=True, row_lab = 'MAI = 0.5', ylab='immigration', annot='D', log=True)
plot_map(AX[3], 'el_sync_map_mai_0.5'+strad+'.csv', mmax=0, mmin=-7, xoff=True, row_lab = 'MAI = 0.5', ylab='immigration', annot='D', log=True)
plot_map(AX[4], 'el_inv_map_mai_0.5'+strad+'.csv', mmax=15, xoff=True,yoff=True, annot='E', log=True)
plot_map(AX[5], 'pop_inv_map_mai_0.5'+strad+'.csv', mmax=10, xoff=True, yoff=True, annot='F',log=True) 

#plot_map(AX[6], 'min_inv_map_mai_1.0'+strad+'.csv', mmax=5, mmin=-9, xlab='habitat loss', ylab='immigration', annot='G', row_lab='MAI = 1.0', log=True)
plot_map(AX[6], 'el_sync_map_mai_1.0'+strad+'.csv', mmax=0, mmin=-7, xlab='habitat loss', ylab='immigration', annot='G', row_lab='MAI = 1.0', log=True)
plot_map(AX[7], 'el_inv_map_mai_1.0'+strad+'.csv', mmax=15, xlab='habitat loss', yoff=True, annot='H', log=True)
plot_map(AX[8], 'pop_inv_map_mai_1.0'+strad+'.csv', mmax=10, xlab='habitat loss', yoff=True, annot='I', log=True)
